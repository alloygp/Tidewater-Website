// HOA / Condo — Hands-on philosophy: 4 cards (config-driven)
function HandsOn() {
  ...
  const ICONS = { eye: <Eye/>, calendar: <Calendar/>, phone: <Phone/>, handshake: <Handshake/> };

  const cfg = (window.PAGE_CONFIG && window.PAGE_CONFIG.handson) || {};
  const eyebrow = cfg.eyebrow || 'How Hands-On Actually Works';
  const titleHtml = cfg.titleHtml || 'Proactive isn\u2019t a slogan.<br/>It\u2019s the way we\u2019re built.';
  const lede = cfg.lede || 'Four operational systems that give you the resources of a large company \u2014 and the personal attention of a small one.';
        ...
        <div className="tw-handson-grid">
          {items.map((it,i) => (
            <article key={i} className="tw-handson-card">
              <div className={`tw-handson-icon ${it.cls || ''}`}>{ICONS[it.icon] || ICONS.eye}</div>
              <h3>{it.title}</h3>
